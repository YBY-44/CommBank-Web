...
import { faDollarSign, faL, IconDefinition } from '@fortawesome/free-solid-svg-icons'
...
export function GoalManager(props: Props) {
  ...
  const IconOnClick = (emoji: BaseEmoji) => {
    if (emoji != null) {
  ...
  return (
    <GoalManagerContainer>
      ...
      <Group>
        <Field name="Icon" icon={faCircle} />
        <EmojiPickerContainer>
          {(icon || props.goal.icon) ? (
            <GoalIcon icon={icon || props.goal.icon} onClick={() => setOpen(!open)}/>
          ) : (
            <AddIconButton hasIcon={props.goal.icon ? true : false} onClick={() => setOpen(!open)} />
          )}
          <EmojiPickerContainers
            isOpen={open}
            hasIcon={props.goal.icon ? true : false}
            onClickChildren={IconOnClick}
          />
        </EmojiPickerContainer>
      </Group>
    </GoalManagerContainer>
...
// type GoalIconContainerProps = { shouldShow: boolean; icon: string }
type EmojiPickerContainerProps = {
  isOpen: boolean
  hasIcon: boolean
  onClickChildren: (emoji: BaseEmoji, event?: React.MouseEvent) => void
}
...
const EmojiPickerContainers = (props: EmojiPickerContainerProps) =>
  props.isOpen ? <EmojiPicker onClick={props.onClickChildren} /> : null
